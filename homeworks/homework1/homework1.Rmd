...
date: 'Due Thursday September 7 at 5pm'
output:
  pdf_document
---
...
yielding two time series of length 500, and plot the results. Compute the sample 
mean and sample variance for each one (to be clear, this just a sample mean of 
all data, over all time, and similarly for the variance), and check that these 
are close to the population mean and variance from Q7 and Q8. Also compute and 
plot the sample auto-correlation function using `acf()`, and check again that it 
agrees with the population auto-correlation function from Q7 and Q8.
